...
def add_constraints(path, future_agent, constraints):
    ...
    for i in range(len(path)):
        if (i != 0) & (path[i-1] != path[i]):
            constraint = {'agent': future_agent, 'loc': [path[i], path[i-1]], 'timestep': i, 'positive': False} # 2.2 edge constraints
            constraints.append(constraint)
        constraint = {'agent': future_agent, 'loc': [path[i]], 'timestep': i, 'positive': False} # 2.1 vertext constraints
        constraints.append(constraint)
...
def additional_constraints(path, future_agent, constraints):
    ...
    constraint = {'agent': future_agent, 'loc': [target, target], 'timestep': len(path)-1, 'positive': False} # special constraint for target constraint: 
